...
theorem theorem4_final_of_top_woven_nat
    ...
    (hW : ∀ U : Finset V,
      ...
      D * T + theorem4B D
        (theorem4RatioCeil (theorem4ScaleMaxRatio (G.induce (U : Set V)) D T)) * T ≤
        HadwigerLean.chromatic (G.induce (U : Set V)) →
  ...
  have hcoeff : 3 * (559 + 276 * fG) ≤ 19683 * (1 + fG) := by
    nlinarith
  have hbudget : (D : ℝ) * (T : ℝ) * (559 + 276 * fF) ≤
      (C : ℝ) * (t : ℝ) * (1 + fG) := by
    calc
      (D : ℝ) * (T : ℝ) * (559 + 276 * fF) ≤
          (D : ℝ) * (3 * (t : ℝ)) * (559 + 276 * fF) := by
            gcongr
      _ ≤ (D : ℝ) * (3 * (t : ℝ)) * (559 + 276 * fG) := by
        gcongr
      _ = ((D : ℝ) * (t : ℝ)) * (3 * (559 + 276 * fG)) := by ring
  ...
  have hFχbase : (D : ℝ) * (T : ℝ) * (553 + 276 * fF) ≤
      (HadwigerLean.chromatic F : ℝ) := by
    nlinarith [hbudget, hχ, hUχreal]
  have hceilF : (sF : ℝ) ≤ fF + 1 :=
    (theorem4_ratio_ceil_lt_add_one fF hfF0).le
  have hFχ : D * T + theorem4B D sF * T ≤
      HadwigerLean.chromatic F := by
    have hh : ((D * T + theorem4B D sF * T : ℕ) : ℝ) ≤
        (HadwigerLean.chromatic F : ℝ) := by
      calc
        ((D * T + theorem4B D sF * T : ℕ) : ℝ) =
            (D : ℝ) * (T : ℝ) * (277 + 276 * (sF : ℝ)) := by
              simp only [theorem4B, Nat.cast_add, Nat.cast_mul, Nat.cast_ofNat]
              ring
        _ ≤ (D : ℝ) * (T : ℝ) * (553 + 276 * fF) := by
          have hcoef : 277 + 276 * (sF : ℝ) ≤ 553 + 276 * fF := by
            nlinarith
          exact mul_le_mul_of_nonneg_left hcoef (by positivity)
        _ ≤ (HadwigerLean.chromatic F : ℝ) := hFχbase
    exact_mod_cast hh
